...
public class FailoverWatcher implements Watcher {
  ...
  public FailoverWatcher(Properties properties, boolean canInitZnode) throws IOException {
    ...
    zkQuorum = properties.getProperty(FailoverServer.ZK_QUORUM, "127.0.0.1:2181");//这里改成测试环境
    ...
    zkAdmin = properties.getProperty(FailoverServer.ZK_ADMIN, "h_consistentTime_admin");
  ...
  /**
   * Initialize the base znodes of consistentTime.
   */
  protected void initZnode() {
	//当zookeeper连接初始化成功之后再创建节点
  ...
  protected void processConnection(WatchedEvent event) {
    ...
    case SyncConnected:
      ...
      try {
        ...
        LOG.error("Error to init ZooKeeper object after sleeping 2000 ms, exit immediately");
        System.exit(0);
      } 
      break;
